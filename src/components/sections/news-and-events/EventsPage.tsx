      ...
      <div className="w-full max-w-[1920px] 2xl:mx-auto px-4 md:px-8 lg:px-16">
        {events.length === 0 ? (
          <div className="flex justify-center py-20">
            <p className="text-white/40 text-normal-base">No events available yet.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-10 max-w-[800px] mx-auto">
            {events.map((event) => (
              <EventCard key={event.id} event={event} typeLabels={typeLabels} />
            ))}
          </div>
        )}
      </div>

      {/* Google Calendar embed — uncomment when calendar URL is ready
      <div className="w-full max-w-[1920px] 2xl:mx-auto px-4 md:px-8 lg:px-16 mt-[80px]">
        <div className="max-w-[1100px] mx-auto">
          <div className="flex flex-col gap-3 mb-8 text-center">
            <p className="text-[#E0C759]/60 text-xs primarybold tracking-[6px] uppercase">{t("events.calendar")}</p>
            <h2 className="text-bold-xl md:text-bold-2xl text-[#E0C759]">{t("events.upcomingEvents")}</h2>
          </div>
