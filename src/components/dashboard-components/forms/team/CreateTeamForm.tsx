...
import {
  ...
  CreateTeamMemberWithFileSchema,
...
export function CreateTeamModal({}: CreateTeamModalProps) {
  const modalRef = useRef<_DashboardFormModalBaseRef>(null);
  return (
    <_DashboardFormModalBase ref={modalRef} buttonTitle={"Create new"}>
      <CreateTeamForm data={null} dashboardModalRef={modalRef} />
    </_DashboardFormModalBase>
  );
}

type FormFieldNames = keyof CreateTeamMemberSchema;
type Props = {
  data?: any;
  dashboardModalRef: RefObject<_DashboardFormModalBaseRef>;
};

export default function CreateTeamForm({ data, dashboardModalRef }: Props) {
  ...
  async function submitFormHandler() {
    ...
    const request: CreateTeamMemberWithFileSchema = {
      data: {
        name: formState.name,
    ...
    await dispatch(
      teamRedux.actions.createTeamMemberAsync({
        data: request,
      })
    ).then(async (responseData: any) => {
      if (responseData.meta.requestStatus === "fulfilled") {
        SuccessToast({
          message: "Your team member was sent successfully",
        });
        ...
        ErrorToast({
          message: "There was an error uploading your team member",
        });
      }
