...
export function CreateArticleModal() {
  ...
  return (
    <_DashboardFormModalBase
      ...
      modalSize={"large"}
    >
      <CreateArticleForm dashboardModalRef={modalRef} />
    </_DashboardFormModalBase>
  );
}

type FormFieldNames = keyof CreateArticleSchema;
type Props = {
  dashboardModalRef: RefObject<_DashboardFormModalBaseRef>;
};

export default function CreateArticleForm({ dashboardModalRef }: Props) {
  ...
  async function submitFormHandler() {
    ...
    try {
      ...
      const request: CreateArticleWithFileSchema = {
        data: {
          name: formState.name,
      ...
      await dispatch(
        articlesRedux.actions.createArticleAsync({
          data: request,
        })
        ...
        if (responseData.meta.requestStatus === "fulfilled") {
          ...
          ErrorToast({
            message: "There was an error uploading your article",
          });
        }
