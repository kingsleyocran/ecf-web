...
export function CreateOpEdModal() {
  ...
  return (
    <_DashboardFormModalBase
      ...
      modalSize={"large"}
    >
      <CreateOpEdForm dashboardModalRef={modalRef} />
    </_DashboardFormModalBase>
  );
}

type FormFieldNames = keyof CreateOpEdSchema;
type Props = {
  dashboardModalRef: RefObject<_DashboardFormModalBaseRef>;
};

export default function CreateOpEdForm({ dashboardModalRef }: Props) {
  ...
  async function submitFormHandler() {
    ...
    try {
      ...
      const request: CreateOpEdWithFileSchema = {
        data: {
          name: formState.name,
      ...
      await dispatch(
        opedsRedux.actions.createOpEdAsync({
          data: request,
        })
      ).then(async (responseData: any) => {
        if (responseData.meta.requestStatus === "fulfilled") {
          SuccessToast({
            message: "Your op-ed was created successfully",
          });
          resetFormState();
          ...
          ErrorToast({
            message: "There was an error uploading your op-ed",
          });
        }
      ...
      ErrorToast({
        message: "There was an error processing your op-ed images",
      });
