...
import { CreateOpEdModal } from "../forms/opeds/CreateOpEdForm";
import UpdateOpEdForm from "../forms/opeds/UpdateOpEdForm";
import DashboardOpEdSeeDetails from "../forms/opeds/DashboardOpEdSeeDetails";

type Props = {};

export default function DashboardViewOpeds({}: Props) {
  const dispatch = useAppDispatch();
  const tableDataStored = useAppSelector(
    opedsRedux.reducers.selectTableListData
  );
  const tableFilterOptions = useAppSelector(
    opedsRedux.reducers.selectTableFilterOptions
  );
  const tableLoadingState = useAppSelector(
    opedsRedux.reducers.selectTableLoadingState
  );
  ...
  async function onPaginationChange(value: PaginationFilterSchema) {
    ...
    dispatch(
      opedsRedux.reducers.updateTableFilterOptionsPagination({ data })
    );
    fetchTableData(true);
  }
  ...
  function onSortingChange(value: any[]) {
    ...
    dispatch(opedsRedux.reducers.updateTableFilterOptionsSorting({ data }));
    fetchTableData(true);
  }
  ...
  const fetchTableData = useCallback(
    async (fetchNew: boolean) => {
      if (fetchNew) {
        ...
        dispatch(opedsRedux.actions.checkBeforeFilterOpEd());
      }
    },
    [dispatch]
  );
  ...
  useEffect(() => {
    if (hasMounted) {
      ...
      dispatch(
        opedsRedux.reducers.updateTableFilterOptions({
          tableFilterOptions,
        })
      );
      fetchTableData(true);
        ...
        <div className="flex-1 w-full h-full">
          <DashboardTable
            ...
            formComponentList={[
              {
                buttonTitle: "Details",
                component: DashboardOpEdSeeDetails,
              },
              {
                buttonTitle: "Edit",
                modalSize: "large",
                component: UpdateOpEdForm,
              },
            ]}
